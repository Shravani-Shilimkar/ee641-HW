...
class HierarchicalDrumVAE(nn.Module):
    def __init__(self, z_high_dim=4, z_low_dim=12):
        ...
        # Prior for z_low given z_high
        self.prior_mu_low = nn.Linear(64, z_low_dim)
        self.prior_logvar_low = nn.Linear(64, z_low_dim)

        # Decoder for z_low to pattern
        self.decoder_low_fc = nn.Linear(z_low_dim + z_high_dim, 512)
    ...
    def encode_hierarchy(self, x):
        ...
        Returns:
            mu_low, logvar_low: Parameters for q(z_low|x)
            mu_high, logvar_high: Parameters for q(z_high|z_low)
            z_low, z_high: Sampled latent variables
        """
        x = x.transpose(1, 2).float()
    ...
    def decode_hierarchy(self, z_high, z_low=None, temperature=1.0):
        ...
        """
        if z_low is None:
            h_prior = self.decoder_high(z_high)
            prior_mu = self.prior_mu_low(h_prior)
            prior_logvar = self.prior_logvar_low(h_prior)
            z_low = self.reparameterize(prior_mu, prior_logvar)

        z_combined = torch.cat((z_high, z_low), dim=-1)
        
        h_decode = self.decoder_low_fc(z_combined)
        
        pattern_logits = self.decoder_low(h_decode)
        pattern_logits = pattern_logits.transpose(1, 2)
        
        return pattern_logits / temperature
    ...
    def forward(self, x, beta=1.0, temperature=1.0):
        ...
        recon = self.decode_hierarchy(z_high, z_low, temperature=temperature)
