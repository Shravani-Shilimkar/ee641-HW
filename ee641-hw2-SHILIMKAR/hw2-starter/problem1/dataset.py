...
class FontDataset(Dataset):
    def __init__(self, data_dir, split='train'):
        ...
        # Access the sample list using the key format "train_samples" or "val_samples"
        sample_key = f"{split}_samples"
        if sample_key not in metadata:
             raise KeyError(f"Metadata file must contain a '{sample_key}' list.")
        self.samples = metadata[sample_key]
    ...
    def __getitem__(self, idx):
        ...
        # Construct the full image path from its components
        # Assumes a structure like: data/fonts/train/A/A_script_0000.png
        ...
        # Use the letter index directly from the metadata
