...
def plot_extrapolation_curves(all_results, save_path):
    ...
    for encoding_type, results in all_results.items():
        ...
        style = styles.get(encoding_type, {'color': 'black', 'marker': 'x', 'linestyle': '-'})
    ...
    plt.axvspan(8, 16, alpha=0.2, color='green', label='Training Range (8-16)')
    ...
    # Set x-axis ticks to match test lengths
    all_lengths = sorted(list(all_results.get('sinusoidal', {}).keys()))
    if all_lengths:
        plt.xticks(all_lengths)
        plt.xlim(0, max(all_lengths) + 10)
    else:
         plt.xlim(0, 270)
...
def visualize_learned_positions(model_path, save_path, max_positions=128):
    """
    ...
    # Ensure max_positions is not larger than what's available
    available_positions = weights.shape[0]
    positions_to_plot = min(max_positions, available_positions)
    weights_to_plot = weights[:positions_to_plot, :]
    ...
    # Show first max_positions positions and all dimensions
    # Include xlabel, ylabel, title, and colorbar
    im = plt.imshow(weights_to_plot, cmap='RdBu_r', aspect='auto')
    plt.xlabel('Embedding Dimension')
    plt.ylabel('Position')
    plt.title(f'Learned Positional Embeddings (First {positions_to_plot} Positions)')
    plt.colorbar(im)
...
def analyze_failure_cases(model, dataloader, device, num_examples=10):
    ...
    for i, failure in enumerate(failures):
        ...
        print(f"  True label: {'Sorted' if failure['true_label'] == 1 else 'Unsorted'}")
        print(f"  Predicted: {'Sorted' if failure['predicted'] == 1 else 'Unsorted'}")
...
def main():
    ...
    # Define output paths based on "Deliverables" structure
    results_dir = Path(args.results_dir)
    extrapolation_dir = results_dir / 'extrapolation' 
    ...
    for encoding_type in encoding_types:
        ...
        # Load model from results/<encoding_type>/best_model.pth
        model_path = results_dir / encoding_type / 'best_model.pth'
        ...
        model = create_model(
            ...
            d_model=128, # NOTE: Hardcoding model params for loading
            num_heads=4,
            num_layers=4,
            d_ff=512,
            max_len=300
    ...
    results_json_path = extrapolation_dir / 'extrapolation_results.json'
    with open(results_json_path, 'w') as f:
        json.dump(all_results, f, indent=2)
    print(f"Saved numerical extrapolation results to {results_json_path}")

    # Plot extrapolation curves
    curves_png_path = extrapolation_dir / 'extrapolation_curves.png'
    ...
    learned_model_path = results_dir / 'learned' / 'best_model.pth'
    # viz_save_path = results_dir / 'learned_position_embeddings.png'
    viz_save_path = extrapolation_dir / 'learned_position_embeddings.png'
    if learned_model_path.exists():
        ...
        print(f"\nSkipping learned position viz, model not found at {learned_model_path}")
